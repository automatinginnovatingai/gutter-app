...
The Gutter App was built to modernize workflows in the gutter industry. After seeing how companies rely on paper-heavy, manual processes to handle installer paperwork, job tracking, and payroll, this app was created to eliminate inefficiencies. It replaces outdated routines with a secure, offline solution that automates payroll calculations, centralizes job data, and generates Excel reports — no cloud dependency, no wasted time, and no paper trails.
...
The Gutter App uses SQL Server Express or Full SQL Server as the central data source. This allows multiple employees to stay up to date with job changes, work tickets, and reporting data in real time.
...
- Weekly payroll calculation based on:  
  ...
  • Hourly wage  
- Auto‑splitting pay across multiple installers per job  
- Full job entry interface with builder, model, lot/block, gutter color, linear footage, and accessory usage  
- Auto‑export to Excel with per‑installer sheets grouped by week  
